...
**Current checkpoint:** `Frontend M2-D / R2`
...
M1: Source / RSS Engine ModernizationはM1-Gまで完了しました。現在は **M2: Frontend Modernization** を進めています。M2-AでFrontend Assetと通信処理の土台、M2-BでFeed取得と状態表示を整理し、M2-CではHTML landmark、Form / Button、Keyboard、Focus、ARIAを改善し、M2-DではMobile 1列・Tablet 2列・Desktop 4列の表示、長い文字列、明示的なRSS削除、Feed再読込、画面内通知を整理しました。R2ではFeed内のStock列が均等幅になる表示回帰と、Drawerが縦に広がりすぎる表示を補正しています。既存のNavbar、4タブ、Feed CRUD、Stock、Settings、公開API、DB、M1 RSS Engineは維持しています。
...
M2-Aの詳細は [`docs/m2-a-implementation.md`](docs/m2-a-implementation.md)、M2-Bは [`docs/m2-b-implementation.md`](docs/m2-b-implementation.md)、M2-Cは [`docs/m2-c-implementation.md`](docs/m2-c-implementation.md)、M2-Dは [`docs/m2-d-implementation.md`](docs/m2-d-implementation.md) を参照してください。test結果は [`docs/test-report-m2-a.md`](docs/test-report-m2-a.md)、[`docs/test-report-m2-b.md`](docs/test-report-m2-b.md)、[`docs/test-report-m2-c.md`](docs/test-report-m2-c.md)、[`docs/test-report-m2-d.md`](docs/test-report-m2-d.md) に記録しています。
...
Secure Baseline以降も、現在のM2-D時点では次を残しています。
...
- Legacy由来のBootstrap / jQuery / Drawer / Font Awesome assets整理、依存Version更新、全Themeのcontrast最終調整はM2-E以降
