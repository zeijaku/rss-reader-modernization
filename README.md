...
- ユーザー登録 / ログイン / ログアウト
- 任意の30日間ログイン維持（固定期限、Token Rotation、Logout／Password変更時失効）
...
- Dashboard Widgetの標準／縦2段表示（Smartphoneは自動高）
...
## Version 1.6 progress

| Stage | 内容 | 状態 |
|---|---|---|
| V1.6-A | Baseline、Swipe、Game Widget、Storage調査 | 完了 |
| V1.6-B | Smartphone Tab Swipe方向Indicator | 完了 |
| V1.6-C | Lights Out基本実装 | 完了 |
| V1.6-D | 状態保持、Storage Recovery、Keyboard、品質調整 | 完了 |
| V1.6-E | Full回帰、Documentation、Package、Version 1.6.0確定 | 完了 |

Version 1.6.0では、Smartphone Tab Swipeの方向Indicatorと、第二のMini Game Widget「Lights Out」を追加しました。Lights Outは5×5盤面、Moves、Reset、新しい問題、Clear、状態復元、Storage Recovery、Keyboard／Focusへ対応します。DB、Migration、SQL、API Route、設定、外部Library、音、通知、Vibrationは追加していません。詳細は[`docs/v1-6-release-implementation.md`](docs/v1-6-release-implementation.md)と[`docs/test-report-v1-6-release.md`](docs/test-report-v1-6-release.md)を参照してください。
...
| Stage | 内容 | 状態 |
|---|---|---|
| V1.7-A | Cache／Login persistence／Widget Grid／Security Header総合設計 | 完了 |
| V1.7-B | GitHub開発再開Baseline | 完了 |
| V1.7-C | Asset Cache Busting一元化 | 完了 |
| V1.7-D | HTTP Cache／Security Header | 完了 |
| V1.7-E | Remember Token DB／Backend | 完了 |
| V1.7-F | 30日間ログインUI／Cookie／Session統合 | 完了 |
| V1.7-G | Widget Grid Prototype | 完了 |
| V1.7-H / R1～R4 | Widget縦2段、RSS件数、Grid互換、日本の祝日 | 完了 |
| V1.7-I | Full回帰、Documentation、Package、Version 1.7.0確定 | 完了 |
