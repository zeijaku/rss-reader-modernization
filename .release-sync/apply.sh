...
if grep -E '(^/|(^|/)\.\.(/|$))' /tmp/v134-files.txt; then
  echo 'Unsafe archive path found.' >&2
  exit 1
fi
if grep -Ev '^(app(/|$)|public(/|$)|database/migrations(/|$))' /tmp/v134-files.txt; then
  echo 'Unexpected archive path found.' >&2
  exit 1
fi
...
curl --fail --location --silent --show-error "$base/LICENSE" -o app/mail/vendor/phpmailer/phpmailer/LICENSE
curl --fail --location --silent --show-error "$base/VERSION" -o app/mail/vendor/phpmailer/phpmailer/VERSION
curl --fail --location --silent --show-error "$base/src/Exception.php" -o app/mail/vendor/phpmailer/phpmailer/src/Exception.php
curl --fail --location --silent --show-error "$base/src/OAuthTokenProvider.php" -o app/mail/vendor/phpmailer/phpmailer/src/OAuthTokenProvider.php
curl --fail --location --silent --show-error "$base/src/PHPMailer.php" -o app/mail/vendor/phpmailer/phpmailer/src/PHPMailer.php
curl --fail --location --silent --show-error "$base/src/SMTP.php" -o app/mail/vendor/phpmailer/phpmailer/src/SMTP.php
...
522bcf0d07be7e00114711db5c9ce2b4d59ac041c5ec36eaadd979d5fa7046  app/mail/vendor/phpmailer/phpmailer/src/SMTP.php
EOF
...
current_marker = '## 現在できること'
mail_bullet = '- Mail WidgetによるIMAP受信／検索／Folder切替、SMTP Plain Text送信／Reply、Sent保存、送信／受信添付\n'
if current_marker in text and mail_bullet.strip() not in text:
    pos = text.index(current_marker) + len(current_marker)
    nl = text.find('\n', pos)
    text = text[:nl+1] + '\n' + mail_bullet + text[nl+1:]
...
'''
if not old.startswith('## 1.34.0 - '):
    changelog.write_text(entry + old, encoding='utf-8')

env = Path('config/.env.example')
...
if 'APP_MAIL_CREDENTIAL_KEY_B64=' not in text:
    if anchor not in text:
        raise SystemExit('config env insertion anchor not found')
    text = text.replace(anchor, mail_env + anchor, 1)
env.write_text(text, encoding='utf-8')

config = Path('docs/configuration.md')
...
if '## Mail Widget（V1.34）' not in text:
    if anchor not in text:
        raise SystemExit('configuration insertion anchor not found')
    text = text.replace(anchor, mail_docs + anchor, 1)
backup_anchor = '- `APP_REMOTE_CREDENTIAL_KEY_B64`を保管するSecret store（Remote Files利用時）'
if '- `APP_MAIL_CREDENTIAL_KEY_B64`を保管するSecret store（Mail利用時）' not in text:
    text = text.replace(backup_anchor, '- `APP_MAIL_CREDENTIAL_KEY_B64`を保管するSecret store（Mail利用時）\n' + backup_anchor, 1)
...
if '## Backend dependency（V1.34）' not in text:
    if anchor not in text:
        raise SystemExit('dependencies insertion anchor not found')
...
hls_row = '| hls.js | 1.6.16 | Apache-2.0 | HLS Widget only: pinned jsDelivr runtime URL with SRI; loaded lazily by `public/js/camera-video-streaming.js` | `licenses/hls.js-1.6.16-Apache-2.0.txt` |'
phpmailer_row = '| PHPMailer | 7.1.1 | LGPL-2.1-only | `app/mail/vendor/phpmailer/phpmailer/` | `app/mail/vendor/phpmailer/phpmailer/LICENSE` |'
if phpmailer_row not in text:
    if hls_row not in text:
        raise SystemExit('THIRD_PARTY_NOTICES table anchor not found')
    text = text.replace(hls_row, hls_row + '\n' + phpmailer_row, 1)
hls_ref = '- hls.js 1.6.16: https://github.com/video-dev/hls.js/releases/tag/v1.6.16'
if '- PHPMailer 7.1.1:' not in text:
    text = text.replace(hls_ref, hls_ref + '\n- PHPMailer 7.1.1: https://github.com/PHPMailer/PHPMailer/tree/v7.1.1', 1)
...
- Separate SMTP credentials use the existing Mail credential master key with a distinct AEAD associated-data domain.
...
- Existing IMAP receive/list/folder/search/body behavior remains the receive path.
...
assert 'PHPMailer' in smtp and 'getSentMIMEMessage' in smtp
assert (root / 'app/mail/mail_sent.php').is_file()
assert (root / 'app/mail/mail_attachment.php').is_file()
assert (root / 'app/mail/mail_received_attachment.php').is_file()
attachment = read('app/mail/mail_attachment.php')
assert '5' in attachment
assert '10485760' in attachment
assert '20971520' in attachment
...
assert notes.startswith('# RSS Reader Modernization 1.34.0')
assert 'Verification limits' in notes
...
python3 <<'PY'
from pathlib import Path
runner = Path('tests/run-current-features.sh')
...
if "Current feature contracts: V1.34 Mail" not in text:
    if anchor not in text:
        raise SystemExit('current feature runner anchor not found')
...
php -l app/mail/mail_account.php
php -l app/mail/mail_api.php
php -l app/mail/mail_attachment.php
php -l app/mail/mail_message.php
php -l app/mail/mail_received_attachment.php
php -l app/mail/mail_reply.php
php -l app/mail/mail_sent.php
php -l app/mail/mail_service.php
php -l app/mail/mail_smtp_client.php
php -l public/api_v1.php
