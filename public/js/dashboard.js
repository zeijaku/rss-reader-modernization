(function ($, window, document) {
    ...
    function removeStockFromActions($button) {
        ...
        if (!/^\d+$/.test(stockId) || $stockCard.length === 0) {
            ...
            showNotice('解除するStockを確認出来ませんでした', 'danger');
            return;
        }
    ...
    function closeArticleActionsMenu(returnFocus) {
        ...
        var trigger = articleActionsTrigger;
        if ($menu.length > 0) {
            $menu.prop('hidden', true).attr('style', '').data('article-url', '').data('article-title', '').data('stock-title', '').data('article-context', '').data('stock-id', '');
        }
    ...
    function openArticleActionsMenu($trigger, focusLast) {
        ...
        var articleContext = articleActionValue($trigger, 'context') === 'stock' ? 'stock' : 'feed';
        var stockId = String($trigger.attr('data-stock-id') || '');
        var stockContext = articleContext === 'stock';
        $menu
            .data('article-url', articleUrl)
            .data('article-title', articleTitle)
            .data('stock-title', articleActionValue($trigger, 'title'))
            .data('article-context', articleContext)
        ...
        $menu.find('.article-action-stock-remove')
            .prop('disabled', !stockContext || !/^\d+$/.test(stockId))
            .attr('aria-disabled', stockContext && /^\d+$/.test(stockId) ? 'false' : 'true');
