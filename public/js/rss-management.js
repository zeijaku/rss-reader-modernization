(function ($, document, window) {
    ...
    function renderFeeds(feeds, healthMap) {
        var $body = $('#rssManagementTableBody').empty(); var $status = $('#rssManagementListStatus'); healthMap = healthMap || {}; ensureHealthHeader(); $('#rssManagementCount').text(feeds.length);
        if (feeds.length === 0) { $('#rssManagementTableWrap').prop('hidden', true); setAlert($status, 'light', '登録されているRSSはありません。'); return; }
        feeds.forEach(function (feed) { var $tr = $('<tr>'); var contentId = String(feed.content_id || ''); $('<td>').text(feed.title || '-').appendTo($tr); $('<td>').append(safeLink(feed.feed_url, feed.feed_url)).appendTo($tr); if (feed.site_url) $('<td>').append(safeLink(feed.site_url, feed.site_url)).appendTo($tr); else $('<td>').text('-').appendTo($tr); $('<td>').text(feed.category_path || '-').appendTo($tr); healthCell(healthMap[contentId]).appendTo($tr); $tr.appendTo($body); });
        $status.prop('hidden', true); $('#rssManagementTableWrap').prop('hidden', false);
    }
    function loadHealthForFeeds(feeds) { return apiPost('feed.health.list').done(function (healthResponse) { var healthRows = healthResponse.data && Array.isArray(healthResponse.data.health) ? healthResponse.data.health : []; var healthMap = {}; healthRows.forEach(function (health) { healthMap[String(health.content_id || '')] = health; }); renderFeeds(feeds, healthMap); }).fail(function () { if (feeds.length > 0) { setAlert($('#rssManagementListStatus'), 'warning', 'RSS一覧は表示していますが、Feed Healthの取得に失敗しました。'); $('#rssManagementTableWrap').prop('hidden', false); } }); }
    function loadFeeds() { return apiPost('opml.list').done(function (feedResponse) { var feeds = feedResponse.data && Array.isArray(feedResponse.data.feeds) ? feedResponse.data.feeds : []; renderFeeds(feeds, {}); if (feeds.length > 0) loadHealthForFeeds(feeds); }).fail(function (xhr) { var message = xhr.responseJSON && xhr.responseJSON.error ? xhr.responseJSON.error.message : 'RSS一覧の取得に失敗しました。'; setAlert($('#rssManagementListStatus'), 'danger', message); $('#rssManagementTableWrap').prop('hidden', true); }); }
    ...
    $.getScript('./js/rss-rules.js?v=1.34.2-dev.6').done(function () { $.getScript('./js/rss-rules-integration.js?v=1.34.2-dev.6'); });
    $(loadFeeds);
