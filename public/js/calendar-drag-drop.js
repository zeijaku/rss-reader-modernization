(function ($, window, document) {
    ...
    function dayDelta(fromDate, toDate) {
        var from = dateObject(fromDate), to = dateObject(toDate);
        return from && to ? Math.round((to.getTime() - from.getTime()) / 86400000) : null;
    }

    function shiftDate(value, delta) {
        var date = dateObject(value);
        if (!date || !Number.isInteger(delta) || Math.abs(delta) > 3660) return '';
        date.setUTCDate(date.getUTCDate() + delta);
        return formatDate(date);
    }

    function shiftRange(start, end, anchor, target) {
        start = validIsoDate(start); end = validIsoDate(end); anchor = validIsoDate(anchor); target = validIsoDate(target);
        var delta = dayDelta(anchor, target);
        if (!start || !end || end < start || !anchor || !target || delta === null) return null;
        return {start: shiftDate(start, delta), end: shiftDate(end, delta), delta: delta};
    }
    ...
    function sourceState(entry, anchorDate) {
        ...
        if (exceptionKind === 'cancelled') return null;
        var start = recurring ? attr(entry, 'data-calendar-occurrence-start-date', attr(entry, 'data-event-start-date', '')) : attr(entry, 'data-event-start-date', '');
        var end = recurring ? attr(entry, 'data-calendar-occurrence-end-date', attr(entry, 'data-event-end-date', start)) : attr(entry, 'data-event-end-date', start);
        start = validIsoDate(start); end = validIsoDate(end); anchorDate = validIsoDate(anchorDate);
        if (!start || !end || end < start || !anchorDate) return null;
        return {
    ...
    function buildMovePlan(state, targetDate) {
        ...
        if (!range || range.delta === 0) return null;
        var payload = {
    ...
    function updateCsrf(xhr) {
        ...
        if (/^[a-f0-9]{64}$/.test(token)) $('meta[name="csrf-token"]').attr('content', token);
    }
    ...
    function showNotice(message, type) {
        ...
        $notice.removeClass('alert-success alert-info alert-danger')
            ...
            .attr('role', type === 'danger' ? 'alert' : 'status').prop('hidden', false).text(text(message));
    }

    function request(plan) {
        return $.ajax({url: plan.endpoint, method: 'POST', cache: false, dataType: 'json', timeout: 4000,
    ...
    function clearDropTarget() { $('.calendar-drag-drop-target').removeClass('calendar-drag-drop-target'); }

    function finishDrag() {
        clearDropTarget(); $('.calendar-event-dragging').removeClass('calendar-event-dragging'); dragState = null;
    }
    ...
    function prepareEntries(root) {
        $(root || document).find('.calendar-event-edit-trigger').each(function () {
            ...
            if (eligible) $entry.attr('data-calendar-drag-ready', '1'); else $entry.removeAttr('data-calendar-drag-ready');
        });
    }

    function schedulePrepare() {
        if (prepareTimer !== null) return;
        prepareTimer = window.setTimeout(function () { prepareTimer = null; prepareEntries(document); }, 0);
    }

    function observeCalendarRedraws() {
        var target = document.getElementById('main-content');
        if (!target || typeof window.MutationObserver !== 'function' || redrawObserver !== null) return;
        redrawObserver = new window.MutationObserver(function (mutations) {
            var hasAddedElement = mutations.some(function (mutation) {
                return mutation.type === 'childList' && Array.prototype.some.call(mutation.addedNodes || [], function (node) { return node && node.nodeType === 1; });
            });
            if (hasAddedElement) schedulePrepare();
        });
    ...
    function onDragStart(event) {
        ...
        var entry = event.target && typeof event.target.closest === 'function' ? event.target.closest('.calendar-event-edit-trigger[data-calendar-drag-ready="1"]') : null;
        if (!entry) return;
        var state = sourceState(entry, calendarDateForElement(entry));
        if (!state) { event.preventDefault(); return; }
        dragState = {entry: entry, state: state}; entry.classList.add('calendar-event-dragging');
        if (event.dataTransfer) {
            event.dataTransfer.effectAllowed = 'move';
            try { event.dataTransfer.setData('text/plain', 'calendar-event:' + state.eventId); } catch (error) {}
        }
    }

    function onDragOver(event) {
        if (!dragState || pending) return;
        var day = event.target && typeof event.target.closest === 'function' ? event.target.closest('.calendar-day[data-calendar-date]') : null;
        if (!day || !validIsoDate(day.getAttribute('data-calendar-date'))) return;
        event.preventDefault(); clearDropTarget(); day.classList.add('calendar-drag-drop-target');
    ...
    function onDrop(event) {
        ...
        if (!targetDate) { finishDrag(); return; }
    ...
    function onClickCapture(event) {
        if (Date.now() < suppressClickUntil) {
            ...
            if (entry) { event.preventDefault(); event.stopImmediatePropagation(); }
        }
    }
    ...
    $(function () { prepareEntries(document); observeCalendarRedraws(); });
