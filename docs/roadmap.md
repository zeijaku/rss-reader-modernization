...
現在は `M2-E / R2`。M2-D / R2の表示と機能を維持したまま、実行時に参照されないFrontend配布物、開発用source、metadata、sprite、重複Assetを整理しました。
