...
def main() -> int:
    parser = argparse.ArgumentParser(description='Verify complete Version 1.8.0 source package.')
    ...
    with zipfile.ZipFile(zip_path) as archive:
        ...
        check('application_version=1.8.0' in build and 'intended_tag=v1.8.0' in build, 'source build metadata targets Version 1.8.0')
        version = archive.read(relative['app/version.php']).decode('utf-8')
        check("APP_VERSION = '1.8.0'" in version and "APP_VERSION_LABEL = 'RSS Reader Modernization 1.8.0'" in version, 'complete ZIP has exact release marker')
