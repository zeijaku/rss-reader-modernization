...
REQUIRED = {
    ...
    'tools/build_complete_package.py', '.github/workflows/v1.19.0-release.yml', 'tests/run-v119f.sh', 'SOURCE_BUILD.txt', 'SOURCE_MANIFEST.sha256',
}
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Verify complete Version 1.19.0 source package.')
    ...
    with zipfile.ZipFile(zip_path) as archive:
        ...
        check('application_version=1.19.0' in build and 'application_label=RSS Reader Modernization 1.19.0' in build and 'intended_release=1.19.0' in build and 'intended_tag=v1.19.0' in build and 'package_status=FINAL' in build and 'publishable=yes' in build, 'source build metadata targets final Version 1.19.0')
        version = archive.read(relative['app/version.php']).decode('utf-8')
        check("APP_VERSION = '1.19.0'" in version and "APP_VERSION_LABEL = 'RSS Reader Modernization 1.19.0'" in version, 'complete ZIP has exact final marker')
