...
def main() -> int:
    ...
    with zipfile.ZipFile(zip_path) as archive:
        ...
        check(metadata.get('intended_release') == '1.22.0', 'release build metadata targets 1.22.0')
        check(metadata.get('intended_tag') == 'v1.22.0', 'release build metadata targets v1.22.0')
        ...
        if metadata.get('package_status') == 'RELEASE_CANDIDATE':
            ...
            check(bool(re.fullmatch(r'1\.22\.0-rc[1-9][0-9]*', metadata.get('application_version', ''))), 'release candidate version format is valid')
        ...
        if metadata.get('package_status') == 'FINAL':
            ...
            check("APP_VERSION = '1.22.0'" in version_text, 'final package has exact 1.22.0 version')
