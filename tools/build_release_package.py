...
def validate_mode(mode: str, version: str, label: str) -> tuple[str, str, str]:
    if mode == 'preview':
        if version != 'M4-E R1' or label != 'Release M4-E / R1':
            fail('preview mode requires M4-E R1 version marker')
        return 'PREVIEW', 'no', 'rss-reader-modernization-1.0.0-preview-m4-e'
    if mode == 'rc':
        if not re.fullmatch(r'1\.0\.0-rc[1-9][0-9]*', version):
            fail('rc mode requires APP_VERSION such as 1.0.0-rc1')
    ...
    if mode == 'final':
        if version != INTENDED_RELEASE or label != 'RSS Reader Modernization 1.0.0':
            fail('final mode requires the exact 1.0.0 version and label')
        return 'FINAL', 'yes', 'rss-reader-modernization-1.0.0'
