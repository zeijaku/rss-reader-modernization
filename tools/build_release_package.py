...
INTENDED_RELEASE = '1.20.0'
INTENDED_TAG = 'v1.20.0'
...
def validate_mode(mode: str, version: str, label: str) -> tuple[str, str, str]:
    if mode == 'preview':
        if not re.fullmatch(r'1\.20\.0-dev\.[1-9][0-9]*', version):
            fail('preview mode requires APP_VERSION such as 1.20.0-dev.9')
        if label != f'RSS Reader Modernization {version}':
            fail('preview mode label does not match APP_VERSION')
        return 'PREVIEW', 'no', 'rss-reader-modernization-1.20.0-preview'
    if mode == 'rc':
        if not re.fullmatch(r'1\.20\.0-rc[1-9][0-9]*', version):
            fail('rc mode requires APP_VERSION such as 1.20.0-rc1')
    ...
    if mode == 'final':
        if version != INTENDED_RELEASE or label != 'RSS Reader Modernization 1.20.0':
            fail('final mode requires the exact 1.20.0 version and label')
        return 'FINAL', 'yes', 'rss-reader-modernization-1.20.0'
