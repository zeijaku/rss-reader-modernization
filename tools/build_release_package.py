...
def validate_mode(mode: str, version: str, label: str) -> tuple[str, str, str]:
    if mode == 'preview':
        ...
        return 'PREVIEW', 'no', 'rss-reader-modernization-1.21.0-preview'
    if mode == 'rc':
    ...
    if mode == 'final':
        if version != INTENDED_RELEASE or label != 'RSS Reader Modernization 1.21.0':
            fail('final mode requires the exact 1.21.0 version and label')
        return 'FINAL', 'yes', 'rss-reader-modernization-1.21.0'
