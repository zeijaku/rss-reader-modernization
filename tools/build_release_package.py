...
def validate_mode(mode: str, version: str, label: str) -> tuple[str, str, str]:
    if mode == 'preview':
        if not re.fullmatch(r'1\.1\.0-dev\.[1-9][0-9]*', version):
            fail('preview mode requires APP_VERSION such as 1.1.0-dev.9')
        if not label.startswith('RSS Reader Modernization V1.1-'):
            fail('preview mode requires a Version 1.1 checkpoint label')
        return 'PREVIEW', 'no', 'rss-reader-modernization-1.1.0-preview'
    if mode == 'rc':
        if not re.fullmatch(r'1\.1\.0-rc[1-9][0-9]*', version):
            fail('rc mode requires APP_VERSION such as 1.1.0-rc1')
    ...
    if mode == 'final':
        if version != INTENDED_RELEASE or label != 'RSS Reader Modernization 1.1.0':
            fail('final mode requires the exact 1.1.0 version and label')
        return 'FINAL', 'yes', 'rss-reader-modernization-1.1.0'
