...
def validate_mode(mode: str, version: str, label: str) -> tuple[str, str, str]:
    if mode == 'preview':
        if not re.fullmatch(r'1\.17\.1-dev\.[1-9][0-9]*', version):
            fail('preview mode requires APP_VERSION such as 1.17.2-dev.9')
        if label != f'RSS Reader Modernization {version}':
            fail('preview mode label does not match APP_VERSION')
        return 'PREVIEW', 'no', 'rss-reader-modernization-1.17.2-preview'
    if mode == 'rc':
        if not re.fullmatch(r'1\.17\.1-rc[1-9][0-9]*', version):
            fail('rc mode requires APP_VERSION such as 1.17.2-rc1')
    ...
    if mode == 'final':
        if version != INTENDED_RELEASE or label != 'RSS Reader Modernization 1.17.2':
            fail('final mode requires the exact 1.17.2 version and label')
        return 'FINAL', 'yes', 'rss-reader-modernization-1.17.2'
