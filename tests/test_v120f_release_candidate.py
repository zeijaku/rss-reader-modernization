...
# RC version / publication boundary.
check('APP_VERSION is exact V1.20.0-RC1', "const APP_VERSION = '1.20.0-rc1';" in version)
check('visible label is exact V1.20.0-RC1', "const APP_VERSION_LABEL = 'RSS Reader Modernization 1.20.0-RC1';" in version)
check('asset revision is exact V1.20.0-RC1', "const APP_ASSET_REVISION = '1.20.0-rc1';" in version)
check('README keeps V1.19.0 stable and exposes V1.20.0-RC1', '**Stable release:** `RSS Reader Modernization 1.19.0`' in readme and '**Current release candidate:** `RSS Reader Modernization 1.20.0-RC1`' in readme)
check('CHANGELOG contains V1.20.0-RC1 entry', '## RSS Reader Modernization 1.20.0-RC1 — 2026-08-23' in changelog)
check('release notes target V1.20.0-RC1', notes.startswith('# RSS Reader Modernization 1.20.0-RC1 Release Notes'))
check('RC release notes explicitly say not final', '正式Releaseではありません' in notes)

# Dynamic asset cache revision. calendar-core stays on its own long-lived V1.9 contract.
for asset in [
    ...
    check(f'lazy asset uses V1.20 RC revision: {asset}', asset + '?v=1.20.0-rc1' in calendar)
check('Camera streaming fallback CSS uses V1.20 RC revision', './css/camera-video-streaming.css?v=1.20.0-rc1' in streaming)
...
check('All RSS Recent API exposes only create/update/delete/fetch actions', all(action in allrss_api for action in ['widget.allrss.create', 'widget.allrss.update', 'widget.allrss.delete', 'widget.allrss.fetch']))
...
# Package tooling / CI / documentation.
check('Runtime builder targets intended V1.20.0/v1.20.0', "INTENDED_RELEASE = '1.20.0'" in rb and "INTENDED_TAG = 'v1.20.0'" in rb and "mode == 'rc'" in rb)
check('Runtime verifier enforces V1.20 RC metadata', "metadata.get('intended_release') == '1.20.0'" in rv and r'1\.20\.0-rc' in rv)
check('Complete builder targets V1.20.0-RC1 non-publishable source', "VERSION = '1.20.0-rc1'" in cb and 'package_status=RELEASE_CANDIDATE' in cb and 'publishable=no' in cb)
check('Complete verifier requires exact V1.20.0-RC1 marker', "VERSION = '1.20.0-rc1'" in cv and "RSS Reader Modernization 1.20.0-RC1" in cv)
check('CI runs V1.20-F gate', 'bash tests/run-v120f.sh' in ci)
check('CI keeps V1.19 compatibility without running exact V1.19 final gate', 'bash tests/run-v119.sh' in ci and 'bash tests/run-v119c.sh' in ci and 'bash tests/run-v119d.sh' in ci and 'bash tests/run-v119f.sh' not in ci)
for rel in [
    'docs/v1-20-f-release-candidate.md', 'docs/v1-20-f-production-checklist.md',
    'docs/v1-20-f-updated-files.md', 'APPLY_NOTE_V1_20_F.md',
    'CHECKLIST_FOR_USER_V1_20_F.md', 'UPDATED_FILES_V1_20_F.md',
]:
    check(f'V1.20-F documentation exists: {rel}', (ROOT / rel).is_file())
