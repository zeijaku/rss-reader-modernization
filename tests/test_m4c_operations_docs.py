...
check('## Release M4-C / R1 — 2026-08-02' in change, 'CHANGELOG retains M4-C entry')
check("APP_VERSION = '1.0.0-rc1'" in version and "APP_VERSION_LABEL = 'RSS Reader Modernization 1.0.0-RC1'" in version, 'visible version is M4-D R1')
check('| Installation / Update / Recovery | PASS |' in gate, 'M4-C release gate is PASS')
check('| Real environment evidence | HOLD |' in gate, 'real environment confirmation remains HOLD')
check('M4-E完了はVersion 1.0.0 Release可を意味しません' in gate, 'later M4 stage does not claim final release readiness')
