...
if len(sys.argv) != 2:
    raise SystemExit('Usage: python tests/test_m4e_checkpoint_package.py <checkpoint.zip>')
zip_path = Path(sys.argv[1]).resolve()
check(zip_path.is_file(), 'M4-E checkpoint ZIP exists')
...
with zipfile.ZipFile(zip_path) as archive:
    ...
    with tempfile.TemporaryDirectory(prefix='rss-m4e-checkpoint-') as tmp:
        archive.extractall(tmp)
        project = Path(tmp) / next(iter(top))
        manifest = project / 'docs/package-manifest-m4-e-r1.txt'
        check(manifest.is_file(), 'M4-E checkpoint manifest exists after extraction')
        ...
        check(set(actual_files) == set(expected) | {'docs/package-manifest-m4-e-r1.txt'}, 'checkpoint manifest file set matches extracted ZIP')
        ...
        for rel in [
            ...
            'docs/m4-e-implementation.md', 'docs/test-report-m4-e.md',
            'tests/test_m4e_release_builder.py', 'tests/test_m4e_release_docs.py',
            'tests/test_m4e_release_process.py', 'LICENSE', 'THIRD_PARTY_NOTICES.md',
        ]:
            check((project / rel).is_file(), f'M4-E checkpoint file is packaged: {rel}')

        version = (project / 'app/version.php').read_text(encoding='utf-8')
        check("APP_VERSION = 'M4-E R1'" in version, 'extracted checkpoint has M4-E version')
        check("APP_VERSION_LABEL = 'Release M4-E / R1'" in version, 'extracted checkpoint has M4-E label')
        check(not (project / 'rss-reader-modernization-1.0.0-preview-m4-e.zip').exists(), 'checkpoint does not embed preview release ZIP')
        ...
        secret_hits = []
        for dirname in ['.github', 'app', 'public', 'config', 'database', 'tools']:
...
print(f'All {checks} M4-E checkpoint package checks passed.')
