...
final class V14cRenderStatement extends PDOStatement
{
    private array $rows = [];
...
final class V14cRenderPDO extends PDO
{
    public function __construct() {}
    public function prepare(string $query, array $options = []): PDOStatement|false { return new V14cRenderStatement($query); }
}
set_db_connection_for_testing(new V14cRenderPDO());
app_session_start();
app_session_login(1);
ob_start();
...
with tempfile.TemporaryDirectory(prefix='v14c-render-') as temp:
...
if failures:
    raise SystemExit(1)
print('All V1.4-C Dashboard render checks passed.')
