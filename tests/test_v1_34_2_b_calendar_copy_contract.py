...
check("selected.value !== 'series'" in copy_js, 'series scope remains distinguishable from occurrence-only copy')
...
check("./js/calendar-copy.js?v=1.34.2-dev.6" in loader, 'Calendar copy module is loaded with the current dev.6 cache key')
occurrence_pos = loader.find("./js/calendar-occurrence.js?v=1.34.2-dev.6")
recurrence_pos = loader.find("./js/calendar-recurrence.js?v=1.34.2-dev.6")
detail_pos = loader.find("./js/calendar-event-details.js?v=1.34.2-dev.6")
copy_pos = loader.find("./js/calendar-copy.js?v=1.34.2-dev.6")
...
check("const APP_VERSION = '1.34.2-dev.6';" in version and "const APP_ASSET_REVISION = '1.34.2-dev.6';" in version, 'application and asset revisions are synchronized at 1.34.2-dev.6')
check('test_v1_34_2_b_calendar_copy_contract.py' in workflow and 'test_v1_34_2_b_calendar_copy.js' in workflow, 'CI executes both static and runtime Calendar copy tests')
