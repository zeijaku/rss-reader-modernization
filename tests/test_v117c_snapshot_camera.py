...
checks = {
    ...
    'Auto mode distinguishes later renderer families': all(token in js for token in ['youtube.com', 'youtu.be', '.m3u8', 'mjpeg']),
    'Auto fallback can attempt Snapshot for extensionless camera endpoints': "return 'snapshot';" in js,
    'Later renderers remain deferred': 'この形式の表示はV1.17-D以降で対応予定です' in js,
    'No iframe renderer was pulled into C': "'<iframe>'" not in js and "createElement('iframe')" not in js,
    'No video element renderer was pulled into C': "'<video>'" not in js and "createElement('video')" not in js,
    ...
    'Camera asset cache marker advances to C': './js/camera-video.js?v=1.17-c' in calendar and './css/camera-video.css?v=1.17-c' in js,
