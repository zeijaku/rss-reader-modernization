...
check(re.search(r"const APP_VERSION = '1\.5\.0(?:-dev\.[12])?';", version) is not None, 'Application Version remains in the V1.5 line')
check('RSS Reader Modernization V1.5-' in version or "APP_VERSION_LABEL = 'RSS Reader Modernization 1.5.0'" in version, 'visible label identifies Version 1.5')
...
check('./css/clock-timer.css' in index and './js/clock-timer.js' in index, 'Timer assets are loaded separately')
