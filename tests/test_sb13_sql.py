...
check(re.search(r"(?:Secure Baseline SB-(?:1[3-9]|[2-9][0-9])|(?:RSS Engine|Frontend|Release) M\d+-[A-Z] / R[1-9][0-9]*|RSS Reader Modernization (?:V\d+\.\d+-[A-Z] / R[1-9][0-9]*|\d+\.\d+\.\d+(?:-(?:RC\d+|dev\.\d+))?))", version) is not None, 'visible version marker is SB-13 or later / M-series / supported version')
