...
check('**Current checkpoint:** `Release M4-D / R1`' in readme, 'README checkpoint is M4-D')
...
check("APP_VERSION = 'M4-D R1'" in version, 'APP_VERSION has progressed to M4-D R1')
check("APP_VERSION_LABEL = 'Release M4-D / R1'" in version, 'APP_VERSION_LABEL has progressed to M4-D R1')
...
for hold in ['GitHub hosted CI / Settings', 'Release ZIP / Notes / SHA-256', 'Real environment / RC', 'Version / Tag / GitHub Release']:
    check(f'| {hold} | HOLD |' in gate, f'later release HOLD remains: {hold}')
check('M4-D完了はVersion 1.0.0 Release可を意味しない' in gate, 'later M4 stage does not falsely claim final release readiness')
