...
check('**Current checkpoint:** `Release M4-B / R1`' in readme, 'README checkpoint is M4-B')
...
check('## Release M4-B / R1 — 2026-08-02' in change, 'CHANGELOG contains M4-B entry')
check("APP_VERSION = 'M4-B R1'" in version, 'APP_VERSION is M4-B R1')
check("APP_VERSION_LABEL = 'Release M4-B / R1'" in version, 'APP_VERSION_LABEL is M4-B R1')
check('| Third-party notice accuracy | PASS |' in gate, 'Third-party notice gate is PASS')
for hold in ['Installation / Update / Recovery', 'GitHub / Portfolio / CI', 'Release ZIP / Notes / SHA-256', 'Real environment / RC', 'Version / Tag / GitHub Release']:
    check(f'| {hold} | HOLD |' in gate, f'later release HOLD remains: {hold}')
check('M4-B完了はVersion 1.0.0 Release可を意味しない' in gate, 'M4-B does not falsely claim final release readiness')
