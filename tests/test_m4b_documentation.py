...
check("APP_VERSION = 'M4-C R1'" in version, 'APP_VERSION is M4-C R1')
check("APP_VERSION_LABEL = 'Release M4-C / R1'" in version, 'APP_VERSION_LABEL is M4-C R1')
...
for hold in ['GitHub / Portfolio / CI', 'Release ZIP / Notes / SHA-256', 'Real environment / RC', 'Version / Tag / GitHub Release']:
    check(f'| {hold} | HOLD |' in gate, f'later release HOLD remains: {hold}')
check('M4-C完了はVersion 1.0.0 Release可を意味しない' in gate, 'later M4 stage does not falsely claim final release readiness')
