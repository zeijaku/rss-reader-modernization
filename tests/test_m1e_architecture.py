...
check("parse_start($entry->body, $source->url, true)" in service, 'cache hit still passes body through Parser and Item Identity scope')
check("parse_start($body, $source->url, true)" in service, 'network body uses the same Parser and Item Identity scope')
...
parse_pos = service.find('$feed = $this->parser->parse_start($body, $source->url, true)')
