...
check('**Current checkpoint:** `RSS Reader Modernization 1.0.0-RC1`' in readme, 'README checkpoint is M4-D')
...
check("APP_VERSION = '1.0.0-rc1'" in version and "APP_VERSION_LABEL = 'RSS Reader Modernization 1.0.0-RC1'" in version, 'visible version has advanced to 1.0.0-rc1')
check('Current: `RSS Reader Modernization 1.0.0-RC1`' in versioning, 'Version policy current marker is M4-E')
check('| GitHub / Portfolio / CI definition | PASS |' in gate, 'M4-D definition gate is PASS')
check('| GitHub hosted CI / Settings | HOLD |' in gate, 'GitHub hosted result remains HOLD until user verification')
check('M4-E完了はVersion 1.0.0 Release可を意味しません' in gate, 'M4-D does not claim final release readiness')
