...
check("const APP_VERSION = '1.34.2-dev.9';" in version, 'visible checkpoint version is dev.8')
check("const APP_VERSION_LABEL = 'RSS Reader Modernization 1.34.2-dev.9';" in version, 'visible version label is dev.8')
check("const APP_ASSET_REVISION = '1.34.2-dev.9';" in version, 'asset revision matches dev.8')
...
check("calendar-copy.js?v=1.34.2-dev.9" in loader, 'B Calendar Copy is present in dev.8 bootstrap')
check("calendar-drag-drop.js?v=1.34.2-dev.9" in loader and "calendar-drag-drop.css?v=1.34.2-dev.9" in loader, 'C Drag & Drop assets are present in dev.8 bootstrap')
check("dashboard-card-wheel.css?v=1.34.2-dev.9" in loader, 'D Card Mouse Wheel stylesheet is present in dev.8 bootstrap')
