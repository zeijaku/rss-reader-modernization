...
if len(sys.argv) != 2:
    raise SystemExit('Usage: python tests/test_m4d_package.py <checkpoint.zip>')
...
with zipfile.ZipFile(zip_path) as archive:
    ...
    with tempfile.TemporaryDirectory(prefix='rss-m4d-package-') as tmp:
        archive.extractall(tmp)
        project = Path(tmp) / next(iter(top))
        manifest = project / 'docs/package-manifest-m4-d-r1.txt'
        check(manifest.is_file(), 'M4-D package manifest exists after extraction')
        ...
        check(set(actual_files) == set(expected) | {'docs/package-manifest-m4-d-r1.txt'}, 'Manifest file set matches extracted ZIP')
        ...
        for rel in [
            ...
            'docs/ci.md', 'docs/github-publication.md', 'docs/portfolio.md',
            'docs/m4-d-implementation.md', 'docs/test-report-m4-d.md',
            'LICENSE', 'THIRD_PARTY_NOTICES.md',
        ]:
            check((project / rel).is_file(), f'M4-D repository/release file is packaged: {rel}')

        version = (project / 'app/version.php').read_text(encoding='utf-8')
        check("APP_VERSION = 'M4-D R1'" in version, 'extracted package has M4-D version')
        check("APP_VERSION_LABEL = 'Release M4-D / R1'" in version, 'extracted package has M4-D label')
...
print(f'All {checks} M4-D package checks passed.')
