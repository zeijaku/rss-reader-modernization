...
with sync_playwright() as p:
    ...
    check(summary_icon.evaluate("el=>el.classList.contains('fa-plus-square')") and icon_style['display'] != 'none' and 12 <= icon_style['size'] <= 16 and icon_style['opacity'] > 0, 'compact Font Awesome plus icon is visibly rendered on smartphone width')
    ...
    overflow = page.locator('.feed-item-title-text').first.evaluate('el=>{const s=getComputedStyle(el);return {scrollWidth:el.scrollWidth,clientWidth:el.clientWidth,scrollHeight:el.scrollHeight,clientHeight:el.clientHeight,mark:el.dataset.feedTitleTruncated,white:s.whiteSpace,lineHeight:parseFloat(s.lineHeight),clamp:s.webkitLineClamp}}')
    check((overflow['scrollWidth'] > overflow['clientWidth'] or overflow['scrollHeight'] > overflow['clientHeight']) and overflow['mark'] == '1', 'only an actually overflowing title is marked truncated')
    check(overflow['white'] == 'normal' and overflow['clamp'] == '2' and overflow['clientHeight'] <= overflow['lineHeight'] * 2.1, 'long title is clamped to at most two lines')
    short_metrics = page.locator('.feed-item-title-text').nth(1).evaluate('el=>{const s=getComputedStyle(el);return {height:el.getBoundingClientRect().height,lineHeight:parseFloat(s.lineHeight),mark:el.dataset.feedTitleTruncated}}')
    ...
    check(first_toggle.get_attribute('aria-expanded') == 'true' and first_toggle.locator('.feed-item-summary-icon').evaluate("el=>el.classList.contains('fa-minus-square')"), 'summary toggle exposes expanded state with a minus icon')
    ...
    first_toggle.click()
    check(page.locator('.feed-item-detail-row').count() == 0 and first_toggle.get_attribute('aria-expanded') == 'false' and first_toggle.locator('.feed-item-summary-icon').evaluate("el=>el.classList.contains('fa-plus-square')"), 'second click closes the summary and restores the plus icon')
