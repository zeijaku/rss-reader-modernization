...
check("const APP_ASSET_REVISION = '1.21-c2';" in version, 'V1.21-C uses a fresh asset revision')
check("loadScript('./js/drawer-categories.js?v=1.21-c2');" in calendar, 'Dashboard / Stock reload the Drawer organizer under the C cache key')
check("./css/drawer-v121c.css?v=1.21-c2" in drawer and 'data-drawer-v121c-style' in drawer, 'Drawer organizer stages the C mobile stylesheet once')
check(drawer.find('./css/drawer-v121b.css?v=1.21-b1') < drawer.find('./css/drawer-v121c.css?v=1.21-c2'), 'V1.21-B visual layer loads before the C mobile adjustments')
...
check('.modal-header > .btn-close {' in mobile_css and 'margin: -10px 0 -10px auto' in mobile_css, 'Smartphone Modal close control is kept inward from the right edge')
check('.modal-header > .btn-close {' in mobile_css and 'width: 44px' in mobile_css and 'height: 44px' in mobile_css, 'Smartphone Modal close control has a 44px touch target')
