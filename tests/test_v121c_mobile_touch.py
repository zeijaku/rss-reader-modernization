...
check("const APP_ASSET_REVISION = '1.21-c1';" in version, 'V1.21-C uses a fresh asset revision')
check("loadScript('./js/drawer-categories.js?v=1.21-c1');" in calendar, 'Dashboard / Stock reload the Drawer organizer under the C cache key')
check("./css/drawer-v121c.css?v=1.21-c1" in drawer and 'data-drawer-v121c-style' in drawer, 'Drawer organizer stages the C mobile stylesheet once')
check(drawer.find('./css/drawer-v121b.css?v=1.21-b1') < drawer.find('./css/drawer-v121c.css?v=1.21-c1'), 'V1.21-B visual layer loads before the C mobile adjustments')
