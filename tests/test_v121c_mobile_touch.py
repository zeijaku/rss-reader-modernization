...
check("const APP_ASSET_REVISION = '1.21.0';" in version, 'V1.21-C uses a fresh asset revision')
check("loadScript('./js/drawer-categories.js?v=1.21.0');" in calendar, 'Dashboard / Stock reload the Drawer organizer under the C cache key')
check("./css/drawer-v121c.css?v=1.21.0" in drawer and 'data-drawer-v121c-style' in drawer, 'Drawer organizer stages the C mobile stylesheet once')
