...
expected_fonts = {
    ...
    for extension in ['eot', 'svg', 'ttf', 'woff', 'woff2']
}
...
check(len(local_css_refs) == 15, 'all.css resolves the expected fifteen local Font Awesome files')
...
for icon in icons:
    check(f'.fa-{icon}:before' in fa_css, f'Font Awesome definition remains for fa-{icon}')
...
license_markers = {
    ...
    PUBLIC / 'css/all.css': 'Font Awesome Free 5.3.1',
    PUBLIC / 'css/drawer.min.css': 'License : MIT',
    PUBLIC / 'js/jquery-3.3.1.min.js': 'jQuery v3.3.1',
...
check(len(public_files) == 39, 'public inventory is reduced from 127 to 39 files')
check(public_size < 5_000_000, 'public inventory is below 5 MB without removing runtime dependencies')
