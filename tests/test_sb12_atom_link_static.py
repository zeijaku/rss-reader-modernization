...
m = re.search(r'public function feed_link\(SimpleXMLElement \$xml\): \?string\s*\{(?P<body>.*?)\n\s*\}\n\n\s*public function feed_description', common, re.S)
body = m.group('body') if m else ''
check(bool(m), 'feed_link implementation is located')
...
check('rss_select_link_candidate($candidates)' in body, 'link relation priority is centralized and tested')
