...
check(bool(re.search(r"(?:Secure Baseline SB-(?:1[2-9]|[2-9]\d+)|(?:RSS Engine|Frontend|Release) M\d+-[A-Z] / R\d+|RSS Reader Modernization V\d+\.\d+-[A-Z] / R\d+|RSS Reader Modernization \d+\.\d+\.\d+(?:-(?:RC\d+|dev\.\d+))?)", version)), 'visible build marker remains SB-12 or later / M-series / supported version')
