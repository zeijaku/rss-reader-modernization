...
check(bool(re.search(r"(?:Secure Baseline SB-(?:1[2-9]|[2-9]\d+)|(?:RSS Engine|Frontend|Release) M\d+-[A-Z] / R\d+|RSS Reader Modernization 1\.0\.0(?:-RC\d+)?|RSS Reader Modernization V1\.1-[A-Z] / R\d+|RSS Reader Modernization 1\.[1-9][0-9]*\.\d+(?:-dev\.\d+)?)", version)), 'visible build marker remains SB-12 or later / M-series / supported version')
