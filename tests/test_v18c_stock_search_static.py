...
check("1.8.0-dev." in version or "APP_VERSION = '1.8.0'" in version, 'V1.8 development or final marker remains present')
...
check('WHERE stock_flag = 0 AND stock_owner = :owner' in search_block, 'Stock search keeps active flag and owner isolation in SQL')
check("stock_title LIKE :stock_title_query ESCAPE '!'" in search_block, 'Stock title search uses its own bound LIKE parameter')
check("stock_data LIKE :stock_data_query ESCAPE '!'" in search_block, 'Stock URL/domain search uses its own bound LIKE parameter')
...
check('記事タイトル / URL / Domain' in stock_branch, 'Search UI documents title, URL, and domain scope')
