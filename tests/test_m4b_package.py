...
if len(sys.argv) != 2:
    raise SystemExit('Usage: python tests/test_m4b_package.py <checkpoint.zip>')
...
with zipfile.ZipFile(zip_path) as archive:
    ...
    with tempfile.TemporaryDirectory(prefix='rss-m4b-package-') as tmp:
        archive.extractall(tmp)
        project = Path(tmp) / next(iter(top))
        manifest = project / 'docs/package-manifest-m4-b-r1.txt'
        check(manifest.is_file(), 'M4-B package manifest exists after extraction')
        expected = {}
        ...
        check(set(actual_files) == set(expected) | {'docs/package-manifest-m4-b-r1.txt'}, 'Manifest file set matches extracted ZIP')
...
print(f'All {checks} M4-B package checks passed.')
