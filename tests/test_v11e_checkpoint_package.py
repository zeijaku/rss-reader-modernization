...
if len(sys.argv) != 2:
    raise SystemExit('usage: test_v11e_checkpoint_package.py <overlay.zip>')

archive = Path(sys.argv[1]).resolve()
assert archive.is_file(), f'ZIP not found: {archive}'
root_name = 'rss-reader-modernization-v1.1-e-r1-overlay/'
required = {
    ...
    root_name + 'tests/test_v11e_widget_reorder.php',
    root_name + 'tests/test_v11e_architecture.py',
    root_name + 'tests/test_v11e_frontend_runtime.js',
    root_name + 'tests/test_v11e_checkpoint_package.py',
    root_name + 'docs/v1-1-e-implementation.md',
    root_name + 'docs/test-report-v1-1-e.md',
    root_name + 'docs/v1-1-e-overlay-manifest.txt',
...
with zipfile.ZipFile(archive) as zf:
    ...
    manifest_name = root_name + 'docs/v1-1-e-overlay-manifest.txt'
...
print(f'PASS: V1.1-E Overlay ZIP structure ({len(names)} entries)')
print('PASS: required Runtime, Test and Documentation files')
print('PASS: V1.1-D postflight R2 correction is included')
