...
check('calendar-month-layout.css?v=1.33.0' in loader, 'connected CSS has the formal F cache key')
check('calendar-month-layout.js?v=1.33.0' in loader, 'placement JavaScript has the formal F cache key')
check("const APP_VERSION = '1.33.0';" in version, 'formal V1.33 release keeps the F visible-version contract')
check("const APP_ASSET_REVISION = '1.33.0';" in version, 'formal V1.33 release keeps the F asset contract')
