...
class Wrapper {
    ...
    prop(k,v) { if(arguments.length===1){const e=this.elements[0]; if(!e)return undefined; if(k==='hidden')return e.hidden; if(k==='disabled')return e.disabled; if(k==='checked')return Boolean(e.checked); return e[k];} this.elements.forEach(e=>{ if(k==='hidden')e.hidden=v; if(k==='disabled')e.disabled=v; if(k==='checked')e.checked=Boolean(v); else e[k]=v; }); return this; }
    ...
    val(v){if(arguments.length===0)return this.elements[0]?String(this.elements[0].value||''):'';this.elements.forEach(e=>e.value=String(v));return this;}
}
...
const handles=[];
for(let i=1;i<=3;i++){const card=new Element('section',{class:'dashboard-widget','data-dashboard-widget-id':String(i),'data-dashboard-widget-sort-order':String(i*10)});const handle=new Element('button',{class:'widget-drag-handle'});const title=new Element('span',{class:'widget-title-text'});title.textValue='Widget '+i;card.appendChild(handle);card.appendChild(title);grid.appendChild(card);handles.push(handle);}
const clockCard=grid.children[0];
clockCard.attrs['data-dashboard-widget-type']='clock';
clockCard.attrs['data-clock-hour-format']='24';
clockCard.attrs['data-clock-show-seconds']='0';
clockCard.attrs['data-clock-show-date']='1';
const clockTime=new Element('time',{class:'clock-time'});
const clockDate=new Element('div',{class:'clock-date'});
clockCard.appendChild(clockTime);clockCard.appendChild(clockDate);
...
function $(arg){
    ...
    if(arg==='[data-dashboard-widget-type="clock"]')return new Wrapper([clockCard]);
...
check(/^\d{1,2}:\d{2}$/.test(clockTime.textValue),'24-hour Clock renders hours and minutes immediately');
check(/^\d{4}-\d{2}-\d{2}T/.test(String(clockTime.attrs.datetime||'')),'Clock time exposes an ISO datetime');
check(clockDate.textValue.length>0&&clockDate.hidden===false,'Clock date renders when enabled');
clockCard.attrs['data-clock-hour-format']='12';
clockCard.attrs['data-clock-show-seconds']='1';
clockCard.attrs['data-clock-show-date']='0';
intervalCalls[0].fn();
...
console.log('All V1.1-F frontend runtime checks passed.');
