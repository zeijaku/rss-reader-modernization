...
final class V14bRenderStatement extends PDOStatement
{
    private array $rows = [];
...
final class V14bRenderPDO extends PDO
{
    public function __construct() {}
    public function prepare(string $query, array $options = []): PDOStatement|false { return new V14bRenderStatement($query); }
}
set_db_connection_for_testing(new V14bRenderPDO());
app_session_start();
app_session_login(1);
ob_start();
...
with tempfile.TemporaryDirectory(prefix='v14b-render-') as temp:
...
check(sum(1 for attrs in cells if attrs.get('tabindex') == '0') == 2, 'each Mock board has one roving focus entry point')
...
if failures:
    raise SystemExit(1)
print('All V1.4-B Dashboard render checks passed.')
